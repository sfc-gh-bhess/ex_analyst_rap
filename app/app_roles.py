...
app_name = "app_roles.py"

# Handle User Login
def do_login():
    if 'user' not in st.session_state:
        if st.session_state.userid in users:
            st.session_state.user = users[st.session_state.userid]
        else:
            st.warning(f'User not found: `{st.session_state.userid}`')

def do_logout():
    del st.session_state['user']
    get_query_exec_result.clear()
    reset_session_state()
...
def login():
    if 'user' not in st.session_state:
        with st.form("Login"):
            ...
            with st.expander("Login hints"):
                st.markdown("Here are some usernames")
                st.dataframe([{"user":k,"tenant":v["tenant"]} for k,v in users.items()])
            st.text_input("User", key="userid")
...
def jwt_generator():
    return JWTGenerator(
        st.secrets.connections.snowflake.account,
        st.secrets.connections.snowflake.user,
        st.secrets.connections.snowflake.private_key_file
    )

# Send API call
def send_snow_api_request(method, path, headers, params, body, request_guid, timeout):
    jwt_gen = jwt_generator()
    headers["Content-Type"] = "application/json"
    headers["Authorization"] = f"Bearer {jwt_gen.get_token()}"
    headers["Accept"] = "application/json"
    url_base = f"https://{st.secrets.connections.snowflake.account}.snowflakecomputing.com"
...
def get_query_exec_result(query: str, tenant) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    """
...
def display_sql_query(
    ...
    with st.expander("Results", expanded=True):
        with st.spinner("Running SQL..."):
            df, err_msg = get_query_exec_result(sql, st.session_state.user["tenant"])
            if df is None:
